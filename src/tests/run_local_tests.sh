...
# Make sure ab_server is already running from previous tests
if ! pgrep -x "ab_server" > /dev/null; then
    echo "Starting AB emulator for auto_sync_reconnect test."
    $TEST_DIR/ab_server --debug --plc=ControlLogix --path=1,0 "--tag=TestBigArray:DINT[2000]" > auto_sync_reconnect_emulator.log 2>&1 &
    EMULATOR_PID=$!
    if [ $? != 0 ]; then
        echo "Unable to start AB emulator for auto_sync_reconnect test!"
        echo "FAILURE"
        let FAILURES++
        continue
    fi
    sleep 2  # Give the emulator time to start
fi

$VALGRIND$TEST_DIR/test_auto_sync_reconnect > "${TEST}_auto_sync_reconnect_test.log" 2>&1
if [ $? != 0 ]; then
    echo "FAILURE"
    let FAILURES++
else
...
# Ensure emulator is running again after test (which pauses it)
killall -CONT ab_server > /dev/null 2>&1
